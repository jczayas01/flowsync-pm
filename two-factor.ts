...
async function listTimeEntries(ctx: ApiContext) {
  ...
  const where: any = {
    ...
    ...(projectId && { projectId }),
    ...(userId    && { userId }),
    ...(billable === "true"  && { isBillable: true }),
    ...(billable === "false" && { isBillable: false }),
    ...(from || to) && { date: {
      ...(from && { gte: from }),
      ...(to   && { lte: to   }),
    }},
...
async function logTimeEntry(ctx: ApiContext) {
  ...
  const amount = data.isBillable ? data.hours * rate : 0
  ...
  await audit(ctx.workspaceId, ctx.userId, "time.logged" as any, "time_entry", entry.id,
    undefined, { hours: data.hours, projectId: data.projectId, billable: data.isBillable })
