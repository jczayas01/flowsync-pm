...
export function WebhooksView({ webhooks:initialWebhooks, workspaceId, role }:{
  ...
  async function createWebhook(e:React.FormEvent) {
    ...
    try {
      ...
      const secret = `whsec_${Math.random().toString(36).slice(2,22)}`
      const newWebhook:Webhook = {
        id:`wh${Date.now()}`, url:form.url, events:form.events,
        isActive:true, secret, createdAt:new Date().toISOString(),
      ...
      setCreating(false)
      setForm({url:"",events:[]})
    } catch { showToast("✗ Failed to create webhook") }
  ...
  async function testWebhook(id:string) {
    ...
    showToast("✓ Test event sent — check your endpoint")
    setTesting(null)
  }
