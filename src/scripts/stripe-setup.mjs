...
// ── OCR add-on pack ──
{
  const product = await stripe.products.create({
    name: "FlowSync PM — OCR Pages Add-on",
    description: "+200 AI-read (OCR) pages per month. Stackable.",
  })
  const price = await stripe.prices.create({
    product: product.id, unit_amount: 1000, currency: "usd",
    recurring: { interval: "month" },
    nickname: "OCR pack monthly",
  })
  console.log("STRIPE_PRICE_OCR_PACK_MONTHLY=" + price.id)
}
