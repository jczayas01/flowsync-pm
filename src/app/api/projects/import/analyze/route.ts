...
const SPEC = `{
  "project": {
    ...
    "outOfScope": "the document's explicitly excluded items — look for an 'Out of scope' / 'Exclusions' / 'Not included' section and summarize its bullets as one paragraph separated by semicolons; null only if no such section exists",
    "background": "the situation/problem context that motivated the project (current-state pain, drivers), or null",
  ...
  try {
    ...
    const result = {
      project: {
        ...
        description: parsed?.project?.description ? String(parsed.project.description).slice(0, 3000) : null,
        outOfScope: parsed?.project?.outOfScope ? String(parsed.project.outOfScope).slice(0, 3000) : null,
        economicImpact: parsed?.project?.economicImpact ? String(parsed.project.economicImpact).slice(0, 3000) : null,
        background: parsed?.project?.background ? String(parsed.project.background).slice(0, 3000) : null,
