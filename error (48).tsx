...
const createProjectSchema = z.object({
  ...
  methodology: z.enum(['WATERFALL', 'AGILE', 'SCRUM']),
  startDate: z.string().datetime().optional().nullable(),
  endDate: z.string().datetime().optional().nullable(),
  budgetTotal: z.number().min(0).optional().default(0),
  currency: z.string().length(3).optional().default('USD'),
  timezone: z.string().optional().default('America/Puerto_Rico'),
...
const listProjectsSchema = z.object({
  ...
  methodology: z.enum(['WATERFALL','AGILE','SCRUM']).optional(),
...
export const GET = withAuth(async (req: NextRequest, ctx: AuthContext) => {
  try {
    ...
    if (!['OWNER', 'ADMIN', 'SUPER_ADMIN'].includes(ctx.role)) {
...
export const POST = withAuth(async (req: NextRequest, ctx: AuthContext) => {
  try {
    ...
    if (templateData) {
      ...
      await seedDefaultStructure(project.id, data.methodology as Methodology)
    }
...
async function seedDefaultStructure(projectId: string, methodology: Methodology) {
  if (methodology === 'WATERFALL') {
    const phases = [
      ...
      { name: 'Monitoring & Control', order: 5 },
      { name: 'Closure', order: 6 },
    ]
    for (const phase of phases) {
      await prisma.phase.create({ data: { projectId, ...phase } })
    }
  }
    ...
    if (member.userId) {
      ...
      if (wsMember) {
        await prisma.projectMember.create({
          data: { projectId, userId: member.userId, role: member.role as any },
