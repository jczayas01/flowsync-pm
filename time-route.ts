...
async function logTimeEntry(ctx: ApiContext) {
  ...
  // Resolve hourly rate: entry override → member cost rate → null.
  // Snapshot at creation so later rate changes never rewrite history; a null
  // rate stays unposted until the member has a costRate (the labor cron
  // resolves it then and leaves truly rate-less entries for later).
  let rate: number | null = data.hourlyRate ?? null
  if (rate == null) {
    const project = await db.project.findUnique({
      where: { id: data.projectId }, select: { workspaceId: true },
    })
    if (project) {
      ...
      if (member?.costRate != null) rate = Number(member.costRate)
    }
  }

  const amount = data.billable && rate != null ? data.hours * rate : 0
