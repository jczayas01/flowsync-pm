...
import { requirePermission, requireCanAssignRole, resolveRole } from "@/lib/rbac/guards"
import type { WorkspaceRole } from "@/lib/rbac/roles"

const updateSchema = z.object({
  role:     z.enum(["ADMIN","SUPER_USER","PROGRAM_MANAGER","PROJECT_MANAGER","TEAM_MEMBER","READ_ONLY","CLIENT"]).optional(),
...
async function updateUser(ctx: ApiContext, params?: Record<string, string>) {
  ...
  if (parsed.data.role) {
    ...
    const roleGuard = requireCanAssignRole(ctx.userRole as any, parsed.data.role as WorkspaceRole)
...
export async function DELETE(req: NextRequest, { params }: { params: { userId: string } }) {
  return withWorkspace(req, removeUser, params, ["ADMIN","SUPER_USER","SYSTEM_ADMIN"] as any)
}
