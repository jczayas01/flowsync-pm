...
const API_ENDPOINTS = [
  {
    ...
    endpoints:[
      { method:"GET",    path:"/api/v1/projects",         desc:"List all projects in the workspace",
        ...
        response:`{ "data": [{ "id": "clx...", "code": "PRJ-001", "name": "EHR Modernization",
...
export function ApiDocsView({ apiKeys:initialKeys, workspaceId, role }:{
  ...
  async function createKey(e:React.FormEvent) {
    ...
    setSaving(true)
    try {
      const rawKey = `pxpm_${Math.random().toString(36).slice(2,12)}${Math.random().toString(36).slice(2,20)}`
      const key:ApiKey = {
        id:`key${Date.now()}`, name:newKey.name,
        prefix:rawKey.slice(0,12)+"…",
        scopes:newKey.scopes, isActive:true,
